    ...
    return (
        <>
            ...
            <SidebarWrapper $isCollapsed={isCollapsed} $isMobileOpen={isMobileOpen}>
                <SidebarHeader>
                    {!isCollapsed && <Logo><Link to="/dashboard">UniResult</Link></Logo>}
                    ...
                    <MenuItem $isSignOut>
                        <button
                            ...
                            style={{
                                ...
                                padding: '0.75rem 1rem', borderRadius: '10px', color: '#dc2626',
                                background: 'transparent', fontSize: '0.95em', fontWeight: '500',
...
const SidebarWrapper = styled.aside`
    ...
    background: var(--white-color); border-right: 1px solid #eee;
    transition: all 0.3s ease; position: fixed; height: 100vh;
    overflow-y: auto; overflow-x: hidden; z-index: 1000; top: 0; left: 0;

    &::-webkit-scrollbar { width: 6px; }
    &::-webkit-scrollbar-track { background: transparent; }
    &::-webkit-scrollbar-thumb { background: var(--stroke-color); border-radius: 3px; }

    @media (max-width: 768px) {
        width: 280px;
        transform: ${(p) => p.$isMobileOpen ? 'translateX(0)' : 'translateX(-100%)'};
        transition: transform 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        box-shadow: ${(p) => p.$isMobileOpen ? '4px 0 8px rgba(0,0,0,0.1)' : 'none'};
...
const SidebarHeader = styled.div`
    ...
    justify-content: space-between; padding: 0 1.5rem; border-bottom: 1px solid #eee;
    @media (max-width: 768px) { padding: 0 1.25rem; }
`

const Logo = styled.h2`
    font-family: 'Cinzel Decorative', serif; font-size: 0.95rem;
    color: var(--primary-color); font-weight: 700; margin: 0; letter-spacing: 0.02em; white-space: nowrap;
...
const CollapseButton = styled.button`
    ...
    border: none; background: var(--primary-color); border-radius: 6px;
    cursor: pointer; color: var(--white-color); transition: all 0.2s; flex-shrink: 0;
    &:hover { opacity: 0.9; }
`
...
const MenuItem = styled.div`
    a, button {
        ...
        border-radius: 10px;
        color: ${(p) => p.$isActive ? 'var(--white-color)' : 'var(--light-text-color)'};
        ...
        transition: all 0.2s; text-decoration: none; width: 100%; box-sizing: border-box;
        &:hover { background: ${(p) => p.$isActive ? 'var(--primary-color)' : '#f3f4f6'}; opacity: ${(p) => p.$isActive ? '0.95' : '1'}; }
        svg { flex-shrink: 0; }
        span { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    }
    ${(p) => p.$isSignOut && `
        margin-top: 2rem; padding-top: 1rem; border-top: 1px solid #eee;
        button { color: #dc2626; &:hover { background: rgba(220, 38, 38, 0.1); } }
    `}
    @media (max-width: 768px) {
        a, button { padding: ${(p) => p.$isSubmenu ? '0.75rem 1rem 0.75rem 2.5rem' : '0.875rem 1rem'}; font-size: ${(p) => p.$isSubmenu ? '0.875rem' : '0.95rem'}; }
    }
`
...
const SettingsToggle = styled.div`
    ...
    border-radius: 10px; color: ${(p) => p.$isActive ? 'var(--primary-color)' : 'var(--light-text-color)'};
    background: ${(p) => p.$isActive ? 'rgba(99, 102, 241, 0.1)' : 'transparent'};
    font-weight: ${(p) => p.$isActive ? '600' : '500'}; font-size: 0.95rem; cursor: pointer; transition: all 0.2s;
    .settings-header { display: flex; align-items: center; gap: 0.75rem; }
    &:hover { background: ${(p) => p.$isActive ? 'rgba(99, 102, 241, 0.15)' : '#f3f4f6'}; }
    svg { flex-shrink: 0; }
    span { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
const SettingsSubmenu = styled.div`
    margin-top: 0.25rem; display: flex; flex-direction: column; gap: 0.25rem;
`
